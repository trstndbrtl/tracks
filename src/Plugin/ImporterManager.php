...
use Drupal\tracks\Plugin\ImporterInterface as ImporterPluginInterface;
...
class ImporterManager extends DefaultPluginManager {
  ...
  public function __construct(\Traversable $namespaces, CacheBackendInterface $cache_backend, ModuleHandlerInterface $module_handler, EntityTypeManager $entityTypeManager) {
    parent::__construct('Plugin/Importer', $namespaces, $module_handler, 'Drupal\tracks\Plugin\ImporterInterface', 'Drupal\tracks\Annotation\Importer');

    $this->alterInfo('tracks_importer_info');
    $this->setCacheBackend($cache_backend, 'tracks_importer_plugins');
