...
namespace Drupal\Tests\tracks\FunctionalJavascript;

use Drupal\file\Entity\File;
use Drupal\FunctionalJavascriptTests\JavascriptTestBase;
use Drupal\tracks\Entity\Importer;
use Drupal\tracks\Entity\TrackType;
...
class ImporterFormTest extends JavascriptTestBase {
  ...
  /**
   * @var \Drupal\tracks\Entity\TrackType
   */
  protected $bundle;
  ...
  public function setUp() {
    parent::setUp();
    $this->container->get('module_installer')->install(['tracks', 'csv_importer_test']);
    $csv_path = drupal_get_path('module', 'csv_importer_test') . '/tracks.csv';
    $csv_contents = file_get_contents($csv_path);
    $this->file = file_save_data($csv_contents, 'public://simpletest-tracks.csv', FILE_EXISTS_REPLACE);
    $this->admin = $this->drupalCreateUser(['administer site configuration']);
    $this->bundle = TrackType::create(['id' => 'goods', 'label' => 'Goods']);
    $this->bundle->save();
  }
  ...
  public function testForm() {
    ...
    $this->assertInstanceOf('Drupal\tracks\Entity\ImporterInterface', $config);
